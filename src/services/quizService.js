...
// const quizEndpointData = require("../../data/quizEndpoint.json");
// const quizSubmissionData = require("../../data/quizSubmissionData.json");
// const apiEndPoint = require("../../data/apiendpoint.json");

const QUIZ_DATA_URL = 'https://www.jsonkeeper.com/b/LLQT';
const QUIZ_SUBMISSION_URL = 'https://api.jsonserve.com/rJvd7g';
const HISTORICAL_QUIZ_DATA_URL = 'https://api.jsonserve.com/XgAgFJ';

const fetchAllQuizData = async () => {
    try {
        const [quizResponse, submissionResponse, historyResponse] = await Promise.all([
            axios.get(QUIZ_DATA_URL),
            axios.get(QUIZ_SUBMISSION_URL),
            axios.get(HISTORICAL_QUIZ_DATA_URL)
        ]);
...
const analyzeUserPerformance = (data) => {
    ...
    Object.keys(userStats).forEach(userId => {
        ...
        user.performanceGaps = Object.keys(user.topicHistory).map(topic => {
            let accuracyHistory = user.topicHistory[topic];
            if (accuracyHistory.length < 3) return null;
